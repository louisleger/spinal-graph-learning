...
# NEW FUNC MAYBE
def laplacians_from_functional_connectivity(functional_connectomes):
    triu_indices = np.triu_indices(functional_connectomes.shape[-1], k=1)
    edge_weights = functional_connectomes[:, triu_indices[0], triu_indices[1]] + 1
    return laplacian_squareform_vec(edge_weights)
...
class GLMM(BaseMixture):
    ...
    def _initialize(self, x, resp):
        ...
        # NEW FUNC MAYBE
        if isinstance(self.laplacian_init, tuple):
            self.weights_ = self.laplacian_init[0]
            self.means_ = self.laplacian_init[1]
            self.laplacians_ = laplacians_from_functional_connectivity(self.laplacian_init[2])
        elif self.laplacian_init is None:
